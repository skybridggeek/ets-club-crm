...
export default function ChatPage() {
  ...
  return (
    // Adjusted Height for Mobile browsers (dvh handles toolbar resizing better)
    <div className="h-[85vh] md:h-[calc(100vh-100px)] flex bg-gray-900 rounded-xl border border-gray-800 overflow-hidden shadow-2xl relative">
      ...
      <div className={`absolute inset-y-0 left-0 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 w-72 bg-gray-950 border-r border-gray-800 flex flex-col transition-transform duration-200 z-50 shadow-2xl md:shadow-none`}>
        <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-950">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="md:hidden text-gray-400 p-2"><X size={20}/></button>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          <button onClick={() => { setActiveChannel("Global"); setIsSidebarOpen(false); }} className={`w-full text-left px-3 py-3 rounded-md flex items-center gap-3 transition ${activeChannel === "Global" ? "bg-brand-primary text-white" : "text-gray-400 hover:bg-gray-800 hover:text-gray-200"}`}>
            <Globe size={18} /> Global Chat
          </button>
          {departments.map(dept => (
            <button key={dept} onClick={() => { setActiveChannel(dept); setIsSidebarOpen(false); }} className={`w-full text-left px-3 py-3 rounded-md flex items-center gap-3 transition ${activeChannel === dept ? "bg-gray-800 text-white border-l-2 border-brand-accent" : "text-gray-400 hover:bg-gray-800 hover:text-gray-200"}`}>
      ...
      <div className="flex-1 flex flex-col bg-gray-900 relative w-full">
        ...
        {/* HEADER (Made lighter to stand out) */}
        <div className="h-16 flex items-center px-4 bg-gray-800 border-b border-gray-700 shadow-md z-30 justify-between shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(true)} className="md:hidden p-2 rounded text-gray-300 hover:bg-gray-700 hover:text-white">
              <Menu size={24} />
            </button>
            <div className="flex items-center gap-2 text-white font-bold text-lg">
              {activeChannel === "Global" ? <Globe size={20} className="text-brand-primary" /> : <Hash size={20} className="text-gray-400" />}
              <span className="truncate max-w-[200px]">{activeChannel}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 text-[10px] text-green-400 uppercase font-bold tracking-wider bg-green-900/20 px-2 py-1 rounded-full border border-green-900/50">
            ...
            messages.map((msg, index) => {
              ...
              return (
                <div key={index} className={`flex gap-3 ${isMe ? "flex-row-reverse" : "flex-row"} group`}>
                  <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-white mt-1 ${showHeader ? (role === 'Member' ? 'bg-gray-700' : 'bg-brand-primary') : 'invisible'}`}>
                    {senderName[0]}
                  </div>
                  <div className={`flex flex-col max-w-[85%] ${isMe ? "items-end" : "items-start"}`}>
                    ...
                    <div className={`px-4 py-2 rounded-2xl text-sm leading-relaxed break-words ${isMe ? "bg-brand-primary text-white rounded-tr-none" : "bg-gray-800 text-gray-200 rounded-tl-none"}`}>
                      {msg.message_text || <span className="italic opacity-50">Image (View on Desktop)</span>}
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="p-3 bg-gray-800 border-t border-gray-700 shrink-0">
          <form onSubmit={handleSendMessage} className="bg-gray-900 rounded-xl flex items-center px-3 py-2 border border-gray-700 focus-within:border-brand-accent transition">
            <div className="p-2 bg-gray-800 rounded-full mr-2 text-gray-500">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-white placeholder-gray-500 outline-none py-1 text-sm"
            />
