...
import { Send, Hash, Globe, Menu, X, RefreshCw, Ghost, MessageSquare, Trash2, AlertTriangle, User, Mail, Phone, Shield, Key } from "lucide-react";
...
export default function ChatPage() {
  ...
  const fetchMessages = async (channelName: string, isBackground = false) => {
    ...
    try {
      ...
      if (msgData && msgData.length > 0) {
        ...
        // FIX: Fetch FULL details so the card isn't empty
        const { data: userData } = await supabase
          .from("users")
          .select("id, full_name, role, club_id, email, phone, department, password_hash") // Added extra fields
          .in("id", senderIds);
  ...
  // --- ACTIONS ---
  const promptDeleteMessage = (msgId: number) => setDeleteMsgId(msgId);
  ...
  return (
    <div className="flex bg-gray-900 rounded-xl border border-gray-800 overflow-hidden shadow-2xl relative min-h-[70vh]">
            ...
            messages.map((msg, index) => {
              ...
              return (
                <div key={index} className={`flex gap-2 sm:gap-3 ${isMe ? "flex-row-reverse" : "flex-row"} group`}>
                  <button 
                    onClick={() => setViewingUser(msg.users)}
                  ...
                  <div className={`flex flex-col max-w-[90%] sm:max-w-[75%] ${isMe ? "items-end" : "items-start"}`}>
                    ...
                    <div className={`px-4 py-2 rounded-2xl text-sm leading-relaxed break-words relative group ${isMe ? "bg-brand-primary text-white rounded-tr-none" : "bg-gray-800 text-gray-200 rounded-tl-none"}`}>
                      {msg.message_text}
                      {isMe && (
                        <button 
                          onClick={() => promptDeleteMessage(msg.id)}
                          className="absolute -left-8 top-1/2 -translate-y-1/2 text-gray-500 hover:text-red-500 p-1 opacity-0 group-hover:opacity-100 transition"
                          title="Delete"
                        >
                          <Trash2 size={14} />
                        </button>
      ...
      {/* --- FULL USER INFO MODAL (Replaced the Alert) --- */}
      {viewingUser && (
            ...
            <div className="px-6 pb-6 relative">
               <div className="-mt-12 mb-4">
                 <div className="w-24 h-24 rounded-full bg-gray-900 p-1 mx-auto">
                   <div className="w-full h-full rounded-full bg-gray-800 flex items-center justify-center text-4xl font-bold text-white border-2 border-brand-accent">
               ...
               <div className="text-center mb-6">
                 <h2 className="text-2xl font-bold text-white">{viewingUser.full_name}</h2>
                 <p className="text-brand-accent font-mono text-sm">{viewingUser.club_id}</p>
                 <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${viewingUser.role === 'Admin' || viewingUser.role === 'Developer' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}>
                    {viewingUser.role}
                 </span>
               </div>

               <div className="bg-black/20 rounded-xl p-4 space-y-3 mb-4 border border-gray-800">
                 {/* Only show Email/Phone if they exist, otherwise show nice fallback */}
                 <div className="flex items-center gap-3 text-gray-300">
                   <div className="p-2 bg-gray-800 rounded-lg text-brand-primary"><Mail size={16} /></div>
                   <div className="flex-1">
                     <p className="text-xs text-gray-500 uppercase">Email</p>
                     <p className="text-sm font-medium select-all">{viewingUser.email || "Not Shared"}</p>
                   </div>
                 </div>
                 
                 <div className="flex items-center gap-3 text-gray-300">
                   <div className="p-2 bg-gray-800 rounded-lg text-green-500"><Phone size={16} /></div>
                   <div className="flex-1">
                     <p className="text-xs text-gray-500 uppercase">Phone</p>
                     <p className="text-sm font-medium select-all">{viewingUser.phone || "Not Shared"}</p>
                   </div>
                 </div>

                 <div className="flex items-center gap-3 text-gray-300">
                   <div className="p-2 bg-gray-800 rounded-lg text-orange-500"><Shield size={16} /></div>
                   <div className="flex-1">
                     <p className="text-xs text-gray-500 uppercase">Department</p>
                     <p className="text-sm font-medium">{viewingUser.department || "General"}</p>
                   </div>
                 </div>
               </div>

               {/* Show Password ONLY to Developer */}
               {user?.role === "Developer" && (
                 <div className="bg-red-900/10 border border-red-900/30 rounded-xl p-4">
                   <div className="flex items-center gap-2 text-red-400 mb-2">
                     <Key size={16} />
                     <p className="text-xs font-bold uppercase tracking-widest">Developer Access</p>
                   </div>
                   <div className="flex items-center justify-between">
                     <p className="text-white font-mono text-lg select-all">{viewingUser.password_hash}</p>
