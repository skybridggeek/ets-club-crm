...
export default function DirectoryPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // SECURITY: If just a member, kick them to profile
    ...
    <div className="space-y-6 relative">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => setViewMode(viewMode === 'active' ? 'banned' : 'active')} className={`px-4 py-2 rounded-lg flex items-center gap-2 transition border ${viewMode === 'banned' ? "bg-red-500/20 text-red-400 border-red-500/50" : "bg-gray-800 text-gray-400 border-gray-700"}`}>{viewMode === 'active' ? <ShieldAlert size={18} /> : <Check size={18} />}{viewMode === 'active' ? 'View Banned' : 'View Active'}</button>
          {viewMode === 'active' && (<button onClick={() => setShowAddForm(true)} className="bg-brand-primary hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition"><UserPlus size={18} /> Add Member</button>)}
        </div>
      </div>
      ...
      <div className={`rounded-xl border overflow-hidden ${viewMode === 'active' ? 'bg-gray-800 border-gray-700' : 'bg-red-950/10 border-red-900/30'}`}>
      ...
      {/* CONFIRM & SUCCESS MODALS (Implicitly kept same logic as previous, just shortened for clarity) */}
