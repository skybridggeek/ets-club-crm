...
export default function FeedbackPage() {
  ...
  // 1. Instead of deleting immediately, we just open the modal
  const promptDelete = (id: number) => {
    setDeleteId(id);
  };

  // 2. The actual delete happens here
  ...
  const canDelete = (item: any) => {
    ...
    if (item.user_id === user.id) {
      ...
      return (currentTime - postTime) / 1000 / 60 <= 10; // 10 mins
    }
    return false;
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 relative">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-400">Voices of the ETS Club. Open and Transparent.</p>
      </div>
        ...
        <form onSubmit={handlePost} className="flex gap-4">
          ...
          <div className="flex-1 space-y-3">
             <textarea
               ...
               className="w-full bg-gray-900 border border-gray-700 rounded-lg p-4 text-white focus:outline-none focus:border-brand-accent min-h-[100px]"
             />
             <div className="flex justify-between items-center">
                ...
                <button 
                  ...
                  className="bg-brand-primary hover:bg-blue-600 text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <div className="space-y-4">
        {feedbacks.map((item) => (
          <div key={item.id} className="bg-gray-800/50 p-6 rounded-xl border border-gray-700/50 hover:border-gray-600 transition group">
             <div className="flex justify-between items-start mb-3">
               <div className="flex items-center gap-3">
                 ...
                 <div>
                   <p className="text-white font-bold text-sm">
                     ...
                     {item.users?.role !== 'Member' && <span className="ml-2 text-[10px] bg-blue-900 text-blue-200 px-1.5 py-0.5 rounded">{item.users?.role}</span>}
                   </p>
               ...
               {canDelete(item) && (
                 <button 
                   ...
                   className="text-gray-500 hover:text-red-500 transition p-2 hover:bg-red-500/10 rounded-full opacity-0 group-hover:opacity-100"
             ...
             <p className="text-gray-300 leading-relaxed pl-[52px]">{item.content}</p>
          </div>
        ))}
        {feedbacks.length === 0 && <div className="text-center py-10 text-gray-500">No feedback yet.</div>}
      </div>
