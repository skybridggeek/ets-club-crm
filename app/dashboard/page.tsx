...
export default function Dashboard() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'post'|'link', id: number } | null>(null);
  ...
  const handlePost = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      // Notify all members
      await notifyAll({
        title: postType === "Meeting" ? "📅 New Meeting Scheduled" : "📢 New Announcement",
        body: postData.title,
        url: "/dashboard",
  ...
  const confirmDelete = async () => {
    ...
    if (deleteTarget.type === 'post') {
      ...
      await supabase.from("quick_links").delete().eq("id", deleteTarget.id);
      fetchLinks();
    }
    ...
    <div className="space-y-6 relative">
      ...
      {isAdmin && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
            <div className="flex items-center justify-between mb-4"><h3 className="text-gray-400 text-sm font-medium">Total Database</h3><div className="p-2 bg-blue-500/10 rounded-lg text-blue-400"><Users size={20} /></div></div>
            <p className="text-3xl font-bold text-white">{stats.total}</p>
          </div>
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
            <div className="flex items-center justify-between mb-4"><h3 className="text-gray-400 text-sm font-medium">Active Members</h3><div className="p-2 bg-green-500/10 rounded-lg text-green-400"><Activity size={20} /></div></div>
            <p className="text-3xl font-bold text-white">{stats.active}</p>
          </div>
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
            <div className="flex items-center justify-between mb-4"><h3 className="text-gray-400 text-sm font-medium">New This Month</h3><div className="p-2 bg-purple-500/10 rounded-lg text-purple-400"><UserPlus size={20} /></div></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          {!loading && announcements.length === 0 && <div className="p-8 bg-gray-800/50 rounded-xl border border-gray-700 text-center text-gray-500">No announcements yet.</div>}
            ...
            <div key={post.id} className="bg-gray-800/50 p-5 rounded-xl border border-gray-700 hover:border-brand-primary/30 transition group">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${post.is_meeting ? 'bg-orange-500/10 text-orange-400' : 'bg-brand-accent/10 text-brand-accent'}`}>{post.is_meeting ? 'Meeting' : 'General'}</span>
                  <span className="text-gray-500 text-xs">{new Date(post.created_at).toLocaleDateString()} • {post.users?.full_name}</span>
                </div>
                {isAdmin && <button onClick={() => setDeleteTarget({type: 'post', id: post.id})} className="text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition"><Trash2 size={16} /></button>}
              </div>
        ...
        <div className="space-y-6">
          <div className="bg-gray-800 p-5 rounded-xl border border-gray-700">
            <div className="flex items-center justify-between mb-4">
               ...
               {isAdmin && <button onClick={() => setShowLinkModal(true)} className="text-xs text-brand-accent hover:text-white transition flex items-center gap-1"><LinkIcon size={12} /> Edit</button>}
            </div>
            <div className="space-y-3">
              {links.map(link => {
                 const IconComponent = getLinkStyle(link.platform).icon;
                 return <a key={link.id} href={link.url} target="_blank" className={`flex items-center justify-center gap-2 w-full ${getLinkStyle(link.platform).color} text-white py-2.5 rounded-lg transition font-medium`}><IconComponent size={18} /> {link.title}</a>
              })}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 p-6 rounded-2xl w-full max-w-lg shadow-2xl relative animate-fade-in">
            ...
            <form onSubmit={handlePost} className="space-y-4">
              <div><label className="block text-xs text-gray-500 uppercase mb-1">Title</label><input required value={postData.title} onChange={e => setPostData({...postData, title: e.target.value})} className="w-full bg-gray-800 p-3 rounded text-white border border-gray-700 focus:border-brand-accent outline-none" /></div>
              {postType === 'Meeting' && (
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 p-6 rounded-2xl w-full max-w-md shadow-2xl relative animate-fade-in">
            ...
            <form onSubmit={handleAddLink} className="space-y-3 mb-6 pb-6 border-b border-gray-800">
               ...
               <input required placeholder="Title" value={newLink.title} onChange={e => setNewLink({...newLink, title: e.target.value})} className="w-full bg-gray-800 p-3 rounded text-white border border-gray-700 focus:border-brand-accent outline-none text-sm" />
               <input required placeholder="URL" value={newLink.url} onChange={e => setNewLink({...newLink, url: e.target.value})} className="w-full bg-gray-800 p-3 rounded text-white border border-gray-700 focus:border-brand-accent outline-none text-sm" />
               <select value={newLink.platform} onChange={e => setNewLink({...newLink, platform: e.target.value})} className="w-full bg-gray-800 p-3 rounded text-white border border-gray-700 focus:border-brand-accent outline-none text-sm">
                 <option value="Generic">Generic Link</option><option value="Telegram">Telegram</option><option value="Zoom">Zoom</option><option value="Drive">Google Drive</option><option value="Instagram">Instagram</option><option value="LinkedIn">LinkedIn</option>
               </select>
              ...
              {links.map(link => {
                 ...
                 return (
                  <div key={link.id} className="flex items-center justify-between bg-black/30 p-3 rounded border border-gray-800">
                     <span className="text-sm text-white flex items-center gap-2"><IconComponent size={14} /> {link.title}</span>
                     <button onClick={() => setDeleteTarget({type: 'link', id: link.id})} className="text-gray-600 hover:text-red-500 transition"><Trash2 size={16} /></button>
                  </div>
                 )
              })}
            </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in">
          <div className="bg-gray-900 border border-gray-700 p-6 rounded-2xl w-full max-w-sm shadow-2xl text-center">
             ...
             <h2 className="text-xl font-bold text-white mb-2">{deleteTarget.type === 'post' ? 'Delete Post?' : 'Remove Link?'}</h2>
             <p className="text-gray-400 text-sm mb-6">This action cannot be undone.</p>
             <div className="flex gap-3">
               <button onClick={() => setDeleteTarget(null)} className="flex-1 py-3 rounded-xl bg-gray-800 text-gray-300 hover:bg-gray-700 font-medium transition">Cancel</button>
